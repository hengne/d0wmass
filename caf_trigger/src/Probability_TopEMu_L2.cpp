...
//          passes the top mu+electrons level2 trigger condition
//----------------------------------------------------------

//Header file for this class
#include "caf_trigger/Probability_TopEMu_L2.hpp"

using namespace std;

Probability_TopEMu_L2::Probability_TopEMu_L2(const char *name)
  : probProcessor(name)
{
...
void Probability_TopEMu_L2::defineEffInfo(std::map< std::string, eff_utils::EffInfo >& _effInfo)
{
...
//--------------------------------------
double Probability_TopEMu_L2::calcProb(std::string version)
{
  //------------------------------------
  ...
  double probability = Prob_TopEMu_L2(version);
...
// P_L2(muon,electron,track) = P_L2(muon) * P_L2(electron) * P_L2(track)
//------------------------------------------------------------------------------
double Probability_TopEMu_L2::Prob_TopEMu_L2(std::string version)
{

  double probability = ( Prob_TopEMu_Muon_L2(version) *
			 Prob_TopEMu_Electron_L2(version) *
			 Prob_TopEMu_Track_L2(version) );

  if ( _debug ) {
    cout << "EMu L2 probability = " << probability << endl;
  }
...
//------------------------------------------------------------------------------
// This calculates the track l2 probability
//------------------------------------------------------------------------------
double Probability_TopEMu_L2::Prob_TopEMu_Track_L2(std::string version){
...
//------------------------------------------------------------------------------
// This calculates the muon l2 probability
//------------------------------------------------------------------------------
double Probability_TopEMu_L2::Prob_TopEMu_Muon_L2(std::string version){
  ...
  //------------------------------------
  // Loop over all muons to get L2 probability
  //------------------------------------
  for(int iobj = 0; iobj < NMU; ++iobj) {
    std::stringstream effInfoName;
    effInfoName << _level << object << "_" << version << "_INC";
    ...
    //    cout << "Level2 muon = " << val << "\t" << effInfoName.str() << endl;
    probArray.push_back( val );
  }
...
//------------------------------------------------------------------------------
// This calculates the electron l2 probability
//------------------------------------------------------------------------------
double Probability_TopEMu_L2::Prob_TopEMu_Electron_L2(std::string version){
  ...
  //------------------------------------
  // Loop over all electrons to get L2 probability
  //------------------------------------
  for(int iobj = 0; iobj < NEM; ++iobj) {
    std::stringstream effInfoName;
...
ClassImp(Probability_TopEMu_L2)
