...
MatchObjectsL3Reco::MatchObjectsL3Reco(const char *name) : cafe::Processor(name) {
    cafe::Config config(name);

    _l3branch = config.get("L3Branch", "");
    _recobranch = config.get("RecoBranch", "");
    _matchname = config.get("Result", "");
    _dphionly =config.get("dPhiMatchOnly", false) ;
    _maxdR = config.get("dR", .5);    
    _minItems = config.get("nMatches", 0);
    _maxItems = config.get("nMatchesMax", 0);
};

bool MatchObjectsL3Reco::processEvent(cafe::Event &event)
{
    bool obj1_L2=false, obj1_L3=false, obj2_L3=false, obj2_Reco=false;
	
    Collection<TMBL3Base> objs1 = event.getCollection<TMBL3Base>(_l3branch);
    Collection<TMBL3Base>::iterator obj1_it=objs1.begin();
    ...
    for(obj1_it=objs1.begin(); obj1_it!=objs1.end(); obj1_it++) {
	  ...
	  if(best_it!=objs2.end()) {
		//_matches.push_back(Match<TMBL3Base,TMBLorentzVector>(obj1_it, _l3branch, best_it, _recobranch, bestdR));
		_matchmap.insert(std::pair<Collection<TMBL3Base>::iterator, Collection<TMBLorentzVector>::iterator>(obj1_it, best_it));
		nmatches++;
		if(debug()>0) {
