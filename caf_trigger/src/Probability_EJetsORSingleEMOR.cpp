...
// Nov 2010 by Joe Haley (cooljoe@fnal.gov)
...
#include "caf_trigger/Probability_EJetsORSingleEMOR.hpp"
#include "eff_utils/EffVal.hpp"

using namespace std;

Probability_EJetsORSingleEMOR::Probability_EJetsORSingleEMOR(const char *name)
  : probProcessor(name)
{
  ...
  //------------------------------------
  _channel = config.get("ChannelName", "EJetsOR");
  ...
  //------------------------------------
  _channel1 = config.get("SaveWeightsName1", "SingleEMOR");
  _channel2 = config.get("SaveWeightsName2", "EJets");
  ...
  _debug = ( debug == "yes" );
  if (_debug) std::cout << std::endl << "Debugging for Probability_EJetsORSingleEMOR is turned on." << std::endl << std::endl;
 ...
 //-----------------------------------------------
  ...
  std::cout << "Probability EJetsORSingleEMOR systematics setting is " << _systematic << std::endl;
...
void Probability_EJetsORSingleEMOR::defineEffInfo(std::map< std::string, eff_utils::EffInfo >& _effInfo)
{

  if(_debug) cout<<"==>Probability_EJetsORSingleEMOR::defineEffInfo(std::map< std::string, eff_utils::EffInfo >& _effInfo)"<<endl;
  //// Update the trigger map (must be defined in children classes)
  change_triggermap();

  if(_debug) cout<<"Create EffInfo request..."<<endl;
  ...
  // Needed for special trigger flags ("SingleEM" and "Group1" like)
  ...
  for ( int i = 0; i < _num_trigger_versions; i++ ) {
	...
	for ( int j = 0; j < _flags_EM.size(); j++ ) {
	  ...
	  if(eff_utils_variable.size()==0) {
	    ...
	    if(flag!="ObjRelativeTo") cerr << "Probability_EJetsORSingleEMOR: " << name() << ": WARNING efficiency"
	  ...
	  if ( flag == "EffName" ) {
	    ...
	    cout << "Probability_EJetsORSingleEMOR: " << name() << " ERROR: internal inconsistency. Ignoring "
		 << flag << endl;
	    exit(1);
	  }
	} // loop over flags
...
//--------------------------------------
double Probability_EJetsORSingleEMOR::calcProb(std::string version)
{

  double prob_e_ejets,prob_e_singleEM,prob_j,prob_and;

//Check consistency
  if ( isRun2b != _v15andAbove ) {
        cout << endl << "Error in Probability_EJetsORSingleEMOR::calcProb :" << endl;
        cout << "Config file says isRun2b is " << _v15andAbove << endl;
        cout << "Event says isRun2b is " << isRun2b << endl;
        exit(1);
  }

  if ( isRun2b ) {	
	prob_e_ejets = Prob_Electron(version, "Group1");
	prob_e_singleEM = Prob_Electron(version, "SingleEM");
	prob_j = Prob_Jet(version, "Group1");
	prob_and = Prob_Electron(version, "SingleEM_AND_Group1");
  }
  else {
	prob_e_ejets = Prob_Electron(version, "EJETS");
	prob_e_singleEM = Prob_Electron(version, "");
	prob_j = Prob_Jet(version, "");
	prob_and = Prob_Electron(version, "AND");
  }	  
  
  if ( _debug ) {
	cout << "\nProbability_EJetsORSingleEMOR::calcProb efficiencies:\n" << endl;
	cout << "\tProb e of ejets: " << prob_e_ejets << endl;
	cout << "\tProb jet of ejets: " << prob_j << endl;
	cout << "\tProb singleEM: " << prob_e_singleEM << endl;
	cout << "\tProb AND: " << prob_and << endl;
	cout << "\tErr of ejets: " << _ejet_uncertainty << endl;
	cout << "\tErr jet of ejets: " << _jet1_uncertainty << endl;
	cout << "\tErr singleEM: " << _singleem_uncertainty << endl;
	cout << "\tErr AND: " << _and_uncertainty << endl;
  }
  ...
  if (prob_and > prob_e_ejets) {
    //_and_is_greater[iversion]++;
    if ( _debug ) {	
	cout << "\tAND is bigger than EJets " << EM[0].Pt() << ' ' << EM[0].Eta() << ' ' << prob_and - prob_e_ejets << endl;
	cout << "\t" << version << ' ' << prob_e_ejets << ' ' << prob_e_singleEM << ' ' << prob_j << ' ' << prob_and << endl;
    }
  }

  if ( _debug ) {
	if (prob_and > prob_e_singleEM) {
		cout << "\tAND is bigger than SingleEM " << version << ' ' << EM[0].Pt() << ' ' << EM[0].Eta() << ' ' << prob_and << ' ' 
			<< prob_e_singleEM << ' ' << prob_and-prob_e_singleEM << endl;
		cout << "\t" << version << ' ' << prob_e_ejets << ' ' << prob_e_singleEM << ' ' << prob_j << ' ' << prob_and << endl;
	}
  }

  //if (prob_and > prob_e_ejets+0.1) _and_is_much_greater[iversion]++;

  if (prob_and > prob_e_ejets) prob_and = prob_e_ejets;
  if (prob_and > prob_e_singleEM) prob_and = prob_e_singleEM;

  double probability = prob_e_singleEM + prob_e_ejets*prob_j - prob_and*prob_j;

  if (probability > 1.) probability = 1.;
  if (probability < 0.) probability = 0.;

  if (_systematic == "nominal") {
    double event_uncertainty = sqrt( _singleem_uncertainty*_singleem_uncertainty + 
				     prob_e_ejets*prob_e_ejets * _jet1_uncertainty*_jet1_uncertainty + 
				     prob_j*prob_j * _ejet_uncertainty*_ejet_uncertainty + 
				     prob_and*prob_and * _jet1_uncertainty*_jet1_uncertainty + 
				     prob_j*prob_j * _and_uncertainty*_and_uncertainty);

    // Above calculation is not correct (it ignores correlations), so set event_uncertainty to 0 so no one thinks it is working right
    event_uncertainty = 0.;
    
//    double event_uncertainty = sqrt( prob_e * prob_e * _jet1_uncertainty * _jet1_uncertainty
//				     + prob_j * prob_j * _e_uncertainty * _e_uncertainty );

     //std::cout << "E uncertainty = " << _e_uncertainty << std::endl;
     //std::cout << "J uncertainty = " << _jet1_uncertainty << std::endl;
     //std::cout << "The " << version << " event probability is " << probability << " +- " << event_uncertainty << std::endl;

     if (version == "16")  { _x13.push_back(probability); _s13.push_back(event_uncertainty); } 
     if (version == "15c")  { _x12.push_back(probability); _s12.push_back(event_uncertainty); } 
     if (version == "15b")  { _x11.push_back(probability); _s11.push_back(event_uncertainty); } 
     if (version == "15a") { _x10.push_back(probability); _s10.push_back(event_uncertainty); } 
     if (version == "14")  { _x9.push_back(probability); _s9.push_back(event_uncertainty); } 
     if (version == "13c") { _x8.push_back(probability); _s8.push_back(event_uncertainty); } 
     if (version == "13b") { _x7.push_back(probability); _s7.push_back(event_uncertainty); } 
     if (version == "13a") { _x6.push_back(probability); _s6.push_back(event_uncertainty); } 
     if (version == "12")  { _x5.push_back(probability); _s5.push_back(event_uncertainty); } 
     if (version == "11")  { _x4.push_back(probability); _s4.push_back(event_uncertainty); } 
     if (version == "10")  { _x3.push_back(probability); _s3.push_back(event_uncertainty); } 
     if (version == "09b")  { _x2.push_back(probability); _s2.push_back(event_uncertainty); } 
     if (version == "09a")  { _x1.push_back(probability); _s1.push_back(event_uncertainty); }      	
  ...
  if ( _debug ) {
    ...
    cout << "\tEJetsORSingleEMOR probability = " << probability << endl;
    cout << endl;
  }
  
  if ( _saveweights ) {
    _savedweights.prob = probability;
    _savedweights.prob1 = prob_e_singleEM;
    _savedweights.prob2 = prob_e_ejets*prob_j;
  }
...
//------------------------------------------------------------------------------
double Probability_EJetsORSingleEMOR::Prob_Electron(string version, string emType) {
 	
  if (_debug ) cout << "\nProbability_EJetsORSingleEMOR::Prob_Electron Run2b = " << isRun2b <<", EMType = " << emType << endl;
  ...
  for (int iobj=0; iobj<NEM; iobj++) {
    ...
    if (_debug) {
      cout << "Probability_EJetsORSingleEMOR::Prob_Electron " << emType << " : object " << iobj << " : " << emprobs[iobj] << endl;
      cout << "Ejets Probability = " << emprobs[iobj] << endl;
    }
  } // loop over electrons
  ...
  if (_debug) {
    cout << "Probability_EJetsORSingleEMOR::Prob_Electron " << emType << " event probability = " << totalProb << endl;
  }
  ...
  if (emType == "SingleEM" || emType == "" )            _singleem_uncertainty = tmpErr;
  if (emType == "Group1"    || emType == "EJETS" )   _ejet_uncertainty = tmpErr;
  if (emType == "SingleEM_AND_Group1" || emType == "AND" ) _and_uncertainty = tmpErr;
  return totalProb;
...
double Probability_EJetsORSingleEMOR::Prob_EM(string version, int iobj,
						   pair<string,eff_utils::EffVal> &weightstore,
						   string emType) {
  ...
  if ( _trigger_map[do_name.str()][0] == "yes" ) {
    ...
    if (_debug) {
      cout << "Probability_EJetsORSingleEMOR::Prob_EM EM probability " << iobj << ": " << val << endl;
    }
    if (val<0) val=0;
    string eff_identifier = prob->Info()->ObjType()+"_"+emType+"_"+prob->Info()->ObjQuality()+"_"+prob->Info()->ObjRelativeTo();
    if ( emType == "" ) eff_identifier = prob->Info()->ObjType()+"_"+prob->Info()->ObjQuality()+"_"+prob->Info()->ObjRelativeTo();
  ...
  //------------------------------------
  if(probproduct > 1.0) {
    cerr << "Probability_EJetsORSingleEMOR::Prob_EM " << name() << " ERROR: EM probability " << probproduct << endl;
    probproduct = 1.0;
  }
...
//------------------------------------------------------------------------------
double Probability_EJetsORSingleEMOR::Prob_Jet(std::string version, string trigtype){

  if ( _debug ) {
    cout << "Probability_EJetsORSingleEMOR::Prob_Jet: EJets (Jets) version = " << version << ", TrigType = " << trigtype << endl;
  }
  ...
  for (int ml=0; ml < _level.size(); ++ml) if (ml == 2 || version == "08" || version == "09a" || version == "09b" || version == "10" || version == "11") {
    std::string level = _level[ml];
    bool valid = false;
  ...
  for(int iobj = 0; iobj < NJET; ++iobj) {
	...
	if (ml == 2 || version == "08" || version == "09a" || version == "09b" || version == "10" || version == "11") {
	  const objectProbabilities *prob = probObject(effInfoName.str().c_str());
	  if ( prob == 0 ) {
	    cout << endl;
	  ...
	  cout.precision(3);
	  if ( _debug ) cout << "Jet\t" << version << "\t" << level << "\t" << JET[iobj].Pt() << "\t" << val << endl;
  ...
  for(int iobj = 0; iobj < NEM; ++iobj) {
	...
	if ( _debug ) {
	  if (threshold == "15") cout << "\tEJets (Jets) L3EM probability T15 = " << prob_t20[2][total_objects + iobj] << endl;
	  ...
	  if (threshold == "30") cout << "\tEJets (Jets) L3EM probability T30 = " << prob_t25[2][total_objects + iobj] << endl;
	}
      }
    }
  }
  ...
  if (version=="14") {
    ...
    if ( _debug ) {
      cout << "\t\tv13c - P0L3: " << P0L3 << endl;
      cout << "\t\tv13c - P1L3: " << P1L3 << endl;
      cout << "\t\tv13c - Pjet: " << Pjet << endl;
      cout << "V13c EJets (Jets) L3 probability = " << Pjet << endl;
    }
	...
	//std::cout << "df_dp[1] " << i+total_objects << " = " << df_dp[i+total_objects][1] << std::endl;
      ...
      for (int i = 0; i < num_of_terms; i++) tmpErr += df_dp[i][0]*df_dp[i][0]*df_dp[i][1]*df_dp[i][1]; 
      tmpErr = sqrt( tmpErr );
      //std::cout << "Prob for jets v13c is = " << Pjet << " +- " << tmpErr << std::endl;
      _jet1_uncertainty = tmpErr;   
    }


  } else if (version=="13a" || version=="13b") {
  ...
  } else if (version == "11" || version=="10" || version=="09a" || version=="09b" || version=="08") {
...
//------------------------------------------------------------------------------
// This calculates the jet l3 probability for jet triggers with 3 jets
// Only needed for certain v15 triggers
//------------------------------------------------------------------------------
double Probability_EJetsORSingleEMOR::Prob_Jet3(std::string version, string trigtype){

  if ( _debug ) {
    cout << "Probability_EJetsORSingleEMOR::Prob_Jet3: EJets (Jets) version = " << version  << ", TrigType = " << trigtype << endl;
  }
  ...
  for (int ml=0; ml < _level.size(); ++ml) if (ml == 2) {
    std::string level = _level[ml];
    bool valid = false;
    for ( int i = 0; i < _location.size(); i++ ) {
      std::string detector = _location[i];
  ...
  double prob_t10[3][100], uncertainty_t10[3][100];
  double prob_t20[3][100], uncertainty_t20[3][100];
  double prob_t25[3][100], uncertainty_t25[3][100];
  ...
  for(int iobj = 0; iobj < NJET; ++iobj) {
	...
	if (ml == 2) {
	  ...
	  // LSC: Fill in the arrays for the probability calculation.
	  if (threshold == "10") { prob_t10[ml][iobj] = val; uncertainty_t10[ml][iobj] = uncertainty; }
	  if (threshold == "20") { prob_t20[ml][iobj] = val; uncertainty_t20[ml][iobj] = uncertainty; }
	  if (threshold == "25") { prob_t25[ml][iobj] = val; uncertainty_t25[ml][iobj] = uncertainty; }
	  if ( _debug ) {
	    if (threshold == "10") cout << "\tEJets (Jets) " << version << " " << level << " probability T10 = " << prob_t10[ml][iobj] << endl;
	    if (threshold == "20") cout << "\tEJets (Jets) " << version << " " << level << " probability T20 = " << prob_t20[ml][iobj] << endl;
	    if (threshold == "25") cout << "\tEJets (Jets) " << version << " " << level << " probability T25 = " << prob_t25[ml][iobj] << endl;
	  }
	}
  ...
  for(int iobj = 0; iobj < NEM; ++iobj) {
	...
	if ( version.substr(0,2) >= "15" ) {
      ...
      for ( int ml = 0; ml < _level.size(); ml++ ) {
	if (threshold == "10") { prob_t10[ml][total_objects + iobj] = 1.0; uncertainty_t10[ml][total_objects + iobj] = 0.0; }
	if (threshold == "20") { prob_t20[ml][total_objects + iobj] = 1.0; uncertainty_t20[ml][total_objects + iobj] = 0.0; }
	if (threshold == "25") { prob_t25[ml][total_objects + iobj] = 1.0; uncertainty_t25[ml][total_objects + iobj] = 0.0; }
      }
	...
	if ( _includeEMshapingAtL3 && ( version.substr(0,2) >= "15" ) ) {
	  if ( _debug) std::cout << "Looking for parameterization : " << effInfoName.str().c_str() << std::endl;	
	  if ( _debug) std::cout << "including shaping " << _threshold[i] << std::endl;
	  ...
	  //cout << "EleJet\t" << version << "\t" << EM[iobj].Pt() << "\t" << val << endl;
	  if ( _debug) cout.precision(3);
	  if ( _debug) cout << "L3EM Jet\t" << version << "\t" << detector_eta << "\t" << EM[iobj].Pt() << "\t" << val << endl;
	  
	  // LSC: Fill in the arrays for the probability calculation.
	  if (threshold == "10") { prob_t10[2][total_objects + iobj] = val; uncertainty_t10[2][total_objects + iobj] = uncertainty; }
	  if (threshold == "20") { prob_t20[2][total_objects + iobj] = val; uncertainty_t20[2][total_objects + iobj] = uncertainty; }
	  if (threshold == "25") { prob_t25[2][total_objects + iobj] = val; uncertainty_t25[2][total_objects + iobj] = uncertainty; }
	}
	
	if ( _debug ) {
	  if (threshold == "10") cout << "\tEJets (Jets) L3EM probability T10 = " << prob_t10[2][total_objects + iobj] << endl;
	  if (threshold == "20") cout << "\tEJets (Jets) L3EM probability T20 = " << prob_t20[2][total_objects + iobj] << endl;
	  if (threshold == "25") cout << "\tEJets (Jets) L3EM probability T25 = " << prob_t25[2][total_objects + iobj] << endl;
	}
      }
  ...
  if (_debug) {
    ...
    for (int i= 0; i < total_objects; i++) {
      cout << " i = " << i << endl;
      cout << "EJets (Jets) L3 probability T10 = " << prob_t10[2][i] << " +- " << uncertainty_t10[2][i] << std::endl;
      cout << "EJets (Jets) L3 probability T20 = " << prob_t20[2][i] << " +- " << uncertainty_t20[2][i] << std::endl;
      cout << "EJets (Jets) L3 probability T25 = " << prob_t25[2][i] << " +- " << uncertainty_t25[2][i] << std::endl;
    }
  }
  ...
  if ( version.substr(0,2) >= "15" ) {
    ...
    // Calculation that at most one of the jets is above the threshold of 25 GeV
    // and at most two of the jets are above 20 GeV
    // and no other jets above 10 GeV.
    for (int i = 0; i < total_objects; i++) {
      P2_factor_1 = prob_t25[2][i];
      for (int j = 0; j < total_objects; j++) {
        if (j!=i) {
	  P2_factor_2 = prob_t20[2][j];
	  P2_factor_3 = 1.0;
	  for (int k = 0; k < total_objects; k++)
	    if (k!=i && k!=j) P2_factor_3 *= 1 - prob_t10[2][k];
	  P2L1 += P2_factor_1 * P2_factor_2 * P2_factor_3;
	}
      }
    }

    // LSC: The probability to fire the trigger is:
    Pjet = 1 - P0L3 - P1L3 - P2L1;
    if ( _debug ) {
      cout << "\t\tv15 - P0L3: " << P0L3 << endl;
      cout << "\t\tv15 - P1L3: " << P1L3 << endl;
      cout << "\t\tv15 - P2L1: " << P2L1 << endl;
      cout << "\t\tv15 - Pjet: " << Pjet << endl;
      cout << "V15 EJets (Jets) L3 probability = " << Pjet << endl;
    }
	...
	df_dp[i][0] = uncertainty_t25[2][i];
	df_dp[i+total_objects][0] = uncertainty_t20[2][i];
	df_dp[i+2*total_objects][0] = uncertainty_t10[2][i];
      }
	...
	df_dp[i+total_objects][1] = P1_factor_1;
      ...
      double P2_factor_1, P2_factor_2, P2_factor_3;
      // Uncertainty for P0L3 from term P2L1
      for (int i = 0; i < total_objects; i++) { 
	P2_factor_1 = 1.0;
	for (int j = 0; j < total_objects; j++) {
	  P2_factor_2 = prob_t20[2][j];
	  if (j!=i) {
	    ...
	    for (int k = 0; k < total_objects; k++)
	      if (k!=i && k!=j) P2_factor_3 *= 1 - prob_t10[2][k];
	    df_dp[i][1] += P2_factor_2 * P2_factor_3;
	  }
	}
      }
      // Uncertainty for P1L3 term from term P2L1
      for (int i = 0; i < total_objects; i++) {
	P2_factor_2 = 1.0;
	for (int j = 0; j < total_objects; j++) {
	  P2_factor_1 = prob_t25[2][j];
	  P2_factor_2 = 1.0;
	  if (j!=i) {
	    P2_factor_3 = 1.0;
	    for (int k = 0; k < total_objects; k++)
	      if ( k!=i && k!=j) P2_factor_3 *= 1 - prob_t10[2][k];
	    df_dp[i+total_objects][1] += P2_factor_1 * P2_factor_3;
	  }
	}
      }
      
      // Uncertainty for P2L1 term from P2L1
      for (int i = 0; i < total_objects; i++) {
	for (int j = 0; j < total_objects; j++) {
	  P2_factor_1 = prob_t25[2][j];
	  for (int k = 0; k < total_objects; k++) {
	    P2_factor_2 = prob_t20[2][k];
	    if (k != j) {
	      P2_factor_3 = 1.0;
	      for (int m = 0; m < total_objects; m++) 
		if (m!=i && m!=j && m!=k) P2_factor_3 *= 1 - prob_t10[2][m];
	      df_dp[i+2*total_objects][1] -= P2_factor_1 * P2_factor_2 * P2_factor_3;
	    }
	  }
	}
      }
      ...
      for (int i = 0; i < num_of_terms; i++) tmpErr += df_dp[i][0]*df_dp[i][0]*df_dp[i][1]*df_dp[i][1]; 
      tmpErr = sqrt( tmpErr );
      //std::cout << "Prob for jets v15 is = " << Pjet << " +- " << tmpErr << std::endl;
      _jet1_uncertainty = tmpErr;   
    }
  }
  ...
  if ( _debug ) {
    cout << "Probability_EJets::Prob_Jet3" << _channel << " Jet Probability = " << probCalc << endl;
  }
...
// // Finish
// //------------------------------------------------------------------------------
void Probability_EJetsORSingleEMOR::PrintEfficiency()
{
...
//   }
  ...
  if (_systematic == "nominal") {
    ...
    if ( _x13.size() != _s13.size() ) std::cout << "Probability_EJetsORSingleEMOR: WARNING - arrays not same size, _x13!= _s13 " << std::endl;
    for (int i=0; i < _x13.size(); ++i) {
      weighted_mean_numerator += _x13[i] / ( _s13[i] * _s13[i] );
      weighted_mean_denominator += 1 / ( _s13[i] * _s13[i] );
      mean_numerator += _x13[i];
      mean_numerator_sigma += _s13[i] * _s13[i];
    }
    weighted_mean[13] = weighted_mean_numerator / weighted_mean_denominator;
    weighted_mean_sigma[13] = sqrt( 1 / weighted_mean_denominator );
    mean[13] = mean_numerator / (_x13.size()+1.e-10);
    mean_sigma[13] = sqrt( mean_numerator_sigma ) / _s13.size();
    if ( mean[13] > 0.0 ) std::cout << "A: Average trigger probability for v16 is " << mean[13] << " +- " << mean_sigma[13] << std::endl;
    ...
    mean_numerator = 0.0;
    mean_numerator_sigma = 0.0;
    if ( _x12.size() != _s12.size() ) std::cout << "Probability_EJetsORSingleEMOR: WARNING - arrays not same size, _x12!= _s12 " << std::endl;
    ...
    if ( mean[12] > 0.0 ) std::cout << "A: Average trigger probability for v15c is " << mean[12] << " +- " << mean_sigma[12] << std::endl;
    ...
    mean_numerator = 0.0;
    mean_numerator_sigma = 0.0;
    if ( _x11.size() != _s11.size() ) std::cout << "Probability_EJetsORSingleEMOR: WARNING - arrays not same size, _x11!= _s11 " << std::endl;
    ...
    if ( mean[11] > 0.0 ) std::cout << "A: Average trigger probability for v15b is " << mean[11] << " +- " << mean_sigma[11] << std::endl;

    weighted_mean_numerator = 0.0;
    weighted_mean_denominator = 0.0;
    mean_numerator = 0.0;
    mean_numerator_sigma = 0.0;
    if ( _x10.size() != _s10.size() ) std::cout << "Probability_EJetsORSingleEMOR: WARNING - arrays not same size, _x10 != _s10 " << std::endl;
    ...
    if ( mean[10] > 0.0 ) std::cout << "A: Average trigger probability for v15a is " << mean[10] << " +- " << mean_sigma[10] << std::endl;
    
    weighted_mean_numerator = 0.0;
    weighted_mean_denominator = 0.0;
    mean_numerator = 0.0;
    mean_numerator_sigma = 0.0;
    if ( _x9.size() != _s9.size() ) std::cout << "Probability_EJetsORSingleEMOR: WARNING - arrays not same size, _x9 != _s9" << std::endl;
    ...
    if ( mean[9] > 0.0 ) std::cout << "A: Average trigger probability for v14 is " << mean[9] << " +- " << mean_sigma[9] << std::endl;

    weighted_mean_numerator = 0.0;
    weighted_mean_denominator = 0.0;
    mean_numerator = 0.0;
    mean_numerator_sigma = 0.0;
    if ( _x8.size() != _s8.size() ) std::cout << "Probability_EJetsORSingleEMOR: WARNING - arrays not same size, _x8 != _s8 " << std::endl;
    ...
    if ( mean[8] > 0.0 ) std::cout << "A: Average trigger probability for v13c is " << mean[8] << " +- " << mean_sigma[8] << std::endl;

    weighted_mean_numerator = 0.0;
    weighted_mean_denominator = 0.0;
    mean_numerator = 0.0;
    mean_numerator_sigma = 0.0;
    if ( _x7.size() != _s7.size() ) std::cout << "Probability_EJetsORSingleEMOR: WARNING - arrays not same size, _x7 != _s7" << std::endl;
    ...
    if ( mean[7] > 0.0 ) std::cout << "A: Average trigger probability for v13b is " << mean[7] << " +- " << mean_sigma[7] << std::endl;

    weighted_mean_numerator = 0.0;
    weighted_mean_denominator = 0.0;
    mean_numerator = 0.0;
    mean_numerator_sigma = 0.0;
    if ( _x6.size() != _s6.size() ) std::cout << "Probability_EJetsORSingleEMOR: WARNING - arrays not same size, _x6 != _s6" << std::endl;
    ...
    if ( mean[6] > 0.0 ) std::cout << "A: Average trigger probability for v13a is " << mean[6] << " +- " << mean_sigma[6] << std::endl;

    weighted_mean_numerator = 0.0;
    weighted_mean_denominator = 0.0;
    mean_numerator = 0.0;
    mean_numerator_sigma = 0.0;
    if ( _x5.size() != _s5.size() ) std::cout << "Probability_EJetsORSingleEMOR: WARNING - arrays not same size, _x5 != _s5" << std::endl;
    ...
    if ( mean[5] > 0.0 ) std::cout << "A: Average trigger probability for v12 is " << mean[5] << " +- " << mean_sigma[5] << std::endl;

    weighted_mean_numerator = 0.0;
    weighted_mean_denominator = 0.0;
    mean_numerator = 0.0;
    mean_numerator_sigma = 0.0;
    if ( _x4.size() != _s4.size() ) std::cout << "Probability_EJetsORSingleEMOR: WARNING - arrays not same size, _x4 != _s4" << std::endl;
    ...
    if ( mean[4] > 0.0 ) std::cout << "A: Average trigger probability for v11 is " << mean[4] << " +- " << mean_sigma[4] << std::endl;

    weighted_mean_numerator = 0.0;
    weighted_mean_denominator = 0.0;
    mean_numerator = 0.0;
    mean_numerator_sigma = 0.0;
    if ( _x3.size() != _s3.size() ) std::cout << "Probability_EJetsORSingleEMOR: WARNING - arrays not same size, _x3 != _s3" << std::endl;
    ...
    if ( mean[3] > 0.0 ) std::cout << "A: Average trigger probability for v10 is " << mean[3] << " +- " << mean_sigma[3] << std::endl;

    weighted_mean_numerator = 0.0;
    weighted_mean_denominator = 0.0;
    mean_numerator = 0.0;
    mean_numerator_sigma = 0.0;
    if ( _x2.size() != _s2.size() ) std::cout << "Probability_EJetsORSingleEMOR: WARNING - arrays not same size, _x2 != _s2" << std::endl;
    ...
    if ( mean[2] > 0.0 ) std::cout << "A: Average trigger probability for v09b is " << mean[2] << " +- " << mean_sigma[2] << std::endl;

    weighted_mean_numerator = 0.0;
    weighted_mean_denominator = 0.0;
    mean_numerator = 0.0;
    mean_numerator_sigma = 0.0;
    if ( _x1.size() != _s1.size() ) std::cout << "Probability_EJetsORSingleEMOR: WARNING - arrays not same size, _x1 != _s1" << std::endl;
    ...
    if ( mean[1] > 0.0 ) std::cout << "A: Average trigger probability for v09a is " << mean[1] << " +- " << mean_sigma[1] << std::endl;

    weighted_mean_numerator = 0.0;
    weighted_mean_denominator = 0.0;
    mean_numerator = 0.0;
    mean_numerator_sigma = 0.0;
    if ( _x0.size() != _s0.size() ) std::cout << "Probability_EJetsORSingleEMOR: WARNING - arrays not same size, _x0 != _s0" << std::endl;
...
void  Probability_EJetsORSingleEMOR::change_triggermap() {

  if(_debug) cout<<"==>Probability_EJetsORSingleEMOR::change_triggermap()"<<endl;
  ...
  //---------------------------------
  // Because the SPCs naming convention changed between v14 and v15
  //---------------------------------
...
// tmp.size() == 0 means that keyword has no value (i.e. non-specified flag), either use default value or throw exception
		...
		if ( version.substr(0,2) > "16" || version.substr(0,2) < "08" ) {
			cerr << "Probability_EJetsORSingleEMOR::Probablity_EJetsORSingleEMOR " << name() << " WARNING: trigger version is out of range: "
				<< version << endl << endl;
		}
	      }
  ...
  // These are for special trigger flags ("SingleEM" and "Group1" like)
  ...
  _EMTrigType.clear();
  if ( _v15andAbove ) {
	_EMTrigType.push_back("SingleEM");
	_EMTrigType.push_back("Group1");
	_EMTrigType.push_back("SingleEM_AND_Group1");
  }
  else {
	_EMTrigType.push_back("AND");
	_EMTrigType.push_back("EJETS");
  }
...
ClassImp(Probability_EJetsORSingleEMOR)
