#include "caf_trigger/MatchL1CalTowerReco.hpp"

MatchL1CalTowerReco::MatchL1CalTowerReco(const char *name) : cafe::Processor(name) {
    cafe::Config config(name);

    _L1Name = config.get("CalTowers", "L1CalTotalTowers");
...
bool MatchL1CalTowerReco::processEvent(cafe::Event &event) 
{
    Collection<TMBL1CalTower> objs1 = event.getCollection<TMBL1CalTower>(_L1Name);
    Collection<TMBL1CalTower>::iterator obj1_it=objs1.begin();
    ...
    for(obj1_it=objs1.begin(); obj1_it!=objs1.end(); obj1_it++) {
	  ...
	  if(best_it!=objs2.end()) {
	    nmatches++ ;
	    _matchmmap.insert(std::pair<Collection<TMBL1CalTower>::iterator, Collection<TMBLorentzVector>::iterator>(obj1_it, best_it));
	    if(debug()>0) {
