...
#include "caf_trigger/Probability_TopAllJets_L1.hpp"

using namespace std;

Probability_TopAllJets_L1::Probability_TopAllJets_L1(const char *name)
  : probProcessor(name)
{
...
void Probability_TopAllJets_L1::defineEffInfo(std::map< std::string, eff_utils::EffInfo >& _effInfo)
{
...
//--------------------------------------
double Probability_TopAllJets_L1::calcProb(std::string version)
{
  //------------------------------------
  // This sets up the channel name and is important to set!
  // This is the value specified to combine in the config file
  //------------------------------------
  _channel = "TopAllJets_L1";
    
  double probability = Prob_TopAllJets_L1(version);
...
//------------------------------------------------------------------------------
double Probability_TopAllJets_L1::Prob_TopAllJets_L1(std::string version)
{

  double probability = ( Prob_TopAllJets_Jet_L1(version) );

  if ( _debug ) {
    cout << "AllJets L1 probability = " << probability << endl;
  }
...
//------------------------------------------------------------------------------
double Probability_TopAllJets_L1::Prob_TopAllJets_Jet_L1(std::string version){
  ...
  if (version=="14") {
    ...
    prob = 1.0 - P1 - P2 - P3;
  } else if (version=="v11" || version=="v10" || version=="v9" || version=="v8") {
	...
	if (iobj!=jobj) {
	  double Pt2 = probArray[jobj];
	  double Ps2 = 0.0;
	  for (int kobj=jobj+1; kobj<length; kobj++){	  
	    if (kobj!=iobj && kobj!=jobj) {
	      double Pt3 = probArray[kobj];
	      double Pt4 = 1.0;
	      for (int lobj=0; lobj<length; lobj++){	  
		if (lobj!=iobj && lobj!=jobj && lobj!=kobj) {
		  Pt4 *= (1.0 - probArray[lobj]);
		}
	      }
	      Ps2 += Pt3 * Pt4;
	    }
...
ClassImp(Probability_TopAllJets_L1)
