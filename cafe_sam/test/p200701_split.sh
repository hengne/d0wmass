...
ln -sf $SRCDIR/p200701.root p200701.root
...
cafe Run: Write\(out\) Input: p200701.root out.File: out_%n.root out.MaxEventsPerFile: 5
STAT=$?
...
# Fail if we didn't get two output files.

if [ ! -f out_0.root -o ! -f out_1.root ]; then
  echo "Didn't get two output files."
  exit 1
fi
