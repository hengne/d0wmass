...
//
void MetSelector::applyCorrection(cafe::Event& event, vector<CAFEMObj*> emobjs, float vtx_old[3], float vtx_new[3], std::map<std::pair<int,int>,int>* liste) {
  
  // revertexing, recalculate missingET for new primary vertex 
  if (liste->size()) _rawMet.SetMet(event, vtx_new, liste);
  else _rawMet.SetMet(event, vtx_new);
