...
TFile *f = new TFile("regions_w.root");
...
//for (Int_t i=16; i<=17; i++){
    ...
    h_low[i]->SetLineColor(4);
    hpeak[i]->SetLineColor(4);
    hhigh[i]->SetLineColor(4);
...
//     h_low[i]->Draw();
//     Double_t normSample = h_low[i]->Integral(); 
//     hpeak[i]->DrawNormalized("Same",normSample);
//     hhigh[i]->DrawNormalized("Same",normSample);
	...
	leg1->Draw();
    c1->Print("threeregions_w_"+var[i]+".eps");
    c1->Print("threeregions_w_"+var[i]+".gif");
...
cout<<"now do:"<<endl;
cout<<".q"<<endl;
cout<<"cp threeregions_w_* ~/next/"<<endl;
